...
class SoftMaxRegression:
    def __call__(self, X):
        ''' Moves through a forward pass of the algorithm.
            Args: 
                X (np.array(shape=(n,m)): A matrix of m rows of training examples with n feature columns
            Returns:
        ...
        return np.exp(linears) * (1 / np.sum(np.exp(linears), axis=1))[:, np.newaxis]
    ...
    def get_params(self):
        return np.vstack((self.weights, self.biases))
    
    def set_params(self, params):
        split_idx = int(self.weights.size / self.weights.shape[1])
        self.weights = params[:split_idx].reshape(self.weights.shape)
        self.biases = params[split_idx:].reshape(self.biases.shape)
